...
// Remove existing artifacts before packaging
...
zip.addLocalFolder(path.resolve(__dirname, '../'), undefined, (filename) => {
  // Exclude development-only files from the packaged extension
  const excludes = ['node_modules', '.git', 'src', 'scripts', 'builtin', '.cdix', 'tsconfig.json', 'vite.config.js'];
  return !excludes.some(ex => filename.includes(ex));
});
zip.writeZip(destFile);

// Create unzipped built-in directory for local development
